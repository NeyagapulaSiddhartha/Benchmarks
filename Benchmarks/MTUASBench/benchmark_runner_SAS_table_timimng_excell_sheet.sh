...
analyse_benchmark() {
    ...
    # Track per-run results for binning: array indexed by valid run number (1-based)
    ...
    local exec_time_total=0   # sum of execution wall-clock seconds (only binary runs)

    # Array to store per-valid-run result: "CAUGHT", "LARGE", "CLEAN"
    ...
    while (( valid_runs < RUNS && total_attempts < max_attempts )); do
        ...
        # --- Time only the binary execution ---
        ...
        ((valid_runs++))
        # Accumulate execution time only for valid runs
    ...
    # --- Build bin stats ---
    # BIN_SIZE=10, compute how many bins
    local BIN_SIZE=10
    local num_bins=$(( (valid_runs + BIN_SIZE - 1) / BIN_SIZE ))
    local bin_str=""   # e.g. "3,2,5" for bins
    local bin_display=""
    ...
    # format: src_path  valid_runs  asan_caught  segfault_count  large_num_count  clean_count  verdict  exec_time_sec  bin_str
    printf '%s\t%d\t%d\t%d\t%d\t%d\t%s\t%s\t%s\n' \
        "$src" "$valid_runs" "$asan_caught" \
        "$segfault_count" "$large_num_count" "$clean_count" \
        "$verdict" "$exec_time_sec" "$bin_str" > "$WORK_DIR/${slug}.stats"
...
for cat_name, dir_fragments in PAPER_CATEGORIES:
    # Collect matching stats entries
    rows = []
    for s in stats:
        if s["verdict"].startswith("SKIPPED_") or s["verdict"] == "COMPILE_ERROR":
            continue
        matched = any(frag in s["src"] for frag in dir_fragments)
        if not matched:
            continue
        rows.append(s)
    ...
    for row_idx, s in enumerate(sorted(rows, key=lambda x: x["fn"]), 2):
        ...
        fill = SKIP_FILL if is_skipped else None
        style_cell(cell, fill=fill, font=BODY_FONT, align=LEFT)

        # Bin columns
        for b in range(NUM_BINS):
            col = b + 2
            if is_skipped or b >= len(s["bins"]):
                val = "-"
            else:
                val = s["bins"][b]
        ...
        # Catch %  — use Excel formula so it stays live
        col_pct = NUM_BINS + 4
        if not is_skipped and s["valid_runs"] > 0:
            ...
            pct_formula = f"=IF({runs_cell}>0,{caught_cell}/{runs_cell},0)"
            cell = ws.cell(row=row_idx, column=col_pct, value=pct_formula)
