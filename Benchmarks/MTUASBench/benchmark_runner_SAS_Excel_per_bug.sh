...
analyse_benchmark() {
    ...
    while (( valid_runs < RUNS && total_attempts < max_attempts )); do
        ...
        if [[ "$asan_class" == "ASAN_CAUGHT" || "$asan_class" == "ASAN_WARNING" ]]; then
            ...
            # ============================================================
            # If extraction failed, dump full ASan output for debugging
            # ============================================================
            if [[ -z "$fault_tid" || "$fault_tid" == "UNKNOWN" ]]; then
                ...
                thread_catch_map["UNKNOWN"]=$(( ${thread_catch_map["UNKNOWN"]:-0} + 1 ))
