...
/* Description: UAS=4 bugs mixed across all patterns.
   BUG 1 (nested deep pass): localVarA is declared in taskLevel0A, passed down to
          taskLevel3A. t1A is joined in main instead of taskLevel0A — UAS.
   BUG 2 (nested deep pass): localVarB is declared in taskLevel0B, passed down to
          taskLevel3B. t1B is joined in main instead of taskLevel0B — UAS.
   BUG 3 (wrong scope join): localVarC is owned by taskLevel1C. t2C is joined in
          main instead of taskLevel1C — UAS.
   BUG 4 (spawn + join in main): localVarD is declared in spawnThreadsD, threads
          tD1 and tD2 both use it; joined in main after spawnThreadsD returns — UAS.
*/
