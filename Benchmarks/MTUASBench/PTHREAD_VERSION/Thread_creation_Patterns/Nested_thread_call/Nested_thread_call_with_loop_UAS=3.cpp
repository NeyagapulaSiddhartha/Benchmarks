...
the object is passed further to task Level4. Else the threads are spawned running function level3ThreadTask. To avoid use After Scope all threads need to be joined 
in taskLevel3 but are joined in main() which leads to potential Use After Scope. */
...
std::vector<pthread_t> threads; // Vector to store thread identifiers
...
void* taskLevel3(void* arg) {
    ...
    for (int i = 10; i < 20; ++i) {
        if (i == 15) {
            ...
            // pthread_join(t4, nullptr);  // Join the level 4 thread immediately
            threads.push_back(t4);  // Store the thread for later joining
        } else {
            pthread_t t3Thread;
        ...
        doWork(1000);  // Sleep to allow threads to start (not a reliable synchronization method)
        
        for(int i=0;i<10;i++)
        {
            for(int j=0;j<10;j++)
            {
                std::cout<<" doing some work in level 3 "<<"\n";
            }
        }
    return nullptr;
}
