...
#     - Excel: "Catch %" column shows per-line breakdown (L56:40%)
#
...
extract_fault_thread() {
    ...
    # LEVEL 5: Symbolization failed — return raw dynamic TID
    echo "$dyn_tid"
}
...
analyse_benchmark() {
    ...
    while (( valid_runs < RUNS && total_attempts < max_attempts )); do
        ...
        if [[ "$asan_class" == "ASAN_CAUGHT" || "$asan_class" == "ASAN_WARNING" ]]; then
            ...
            # --------------------------------------------------------
            # CHANGED: Reject bare T[0-9]+ (no line number resolved),
            # DOUBLE_JOIN, DATA_RACE, UNKNOWN, and empty — all treated
            # as unresolvable. Do NOT count as a valid catch.
            # --------------------------------------------------------
            if [[ -z "$fault_tid" || \
                  "$fault_tid" == "UNKNOWN" || \
                  "$fault_tid" == "DOUBLE_JOIN" || \
                  "$fault_tid" == "DATA_RACE" ]] || \
               echo "$fault_tid" | grep -qE '^T[0-9]+$'; then
                ...
                unknown_file="$OUTDIR/unknown_thread_reports/${slug}_run${valid_runs}.txt"
                {
                    ...
                    echo "TIMESTAMP : $(date)"
                    echo "FAULT_TID : ${fault_tid:-<empty>}"
            ...
            # Normal path — thread ID with line number is known
            ((asan_caught++))
            run_results+=("CAUGHT")
    ...
    # --- Build thread breakdown string e.g. "T1@L35:4,T2@L44:1" ---
    local thread_str=""
...
THREAD_FILL = PatternFill("solid", start_color="FFF2CC")
...
def compute_per_line_catch_pct(thread_str, valid_runs):
    """
    Given thread_str like "T1@L56:3,T2@L56:1,T3@L38:2" and valid_runs=10,
    aggregate counts by line number and return string like "L38:20%, L56:40%".
    Returns "0%" if no thread data or no catches.
    """
    if not thread_str or valid_runs == 0:
        return "0%"
    line_counts = {}
    for entry in thread_str.split(","):
        entry = entry.strip()
        if not entry:
            continue
        # entry format: "T1@L56:3" or "T2@L56:1"
        ...
        if "@L" in tid_part:
            ...
            # bare TID with no line — skip (should not reach here after bash fix)
            continue
    ...
    for line_label in sorted(line_counts.keys(),
        ...
        pct = round(line_counts[line_label] * 100 / valid_runs)
...
for cat_name, dir_fragments in PAPER_CATEGORIES:
    ...
    # CHANGED: header now says "Catch %\n(per line)"
    headers += ["Total\nCaught", "Total\nRuns", "Catch %\n(per line)", "Exec Time (s)", "Fault Threads\n(TID:count)"]
    ...
    for row_idx, s in enumerate(sorted(rows, key=lambda x: x["fn"]), 2):
        ...
        # CHANGED: per-line catch % breakdown
        pct_val = compute_per_line_catch_pct(
            s["thread_str"] if not is_skipped else "",
            s["valid_runs"] if not is_skipped else 0
        )
    ...
    # CHANGED: wider to fit "L56:40%, L34:20%" style content
    ws.column_dimensions[get_column_letter(NUM_BINS + 4)].width = 28
...
# ============================================================
# UAS sheets — grouped by UAS=N in filename
# ============================================================
import re
...
for uas_n in sorted(uas_groups.keys()):
    ...
    # Header row — same as category sheets
    headers = ["Benchmark"]
    for b in range(1, NUM_BINS + 1):
        start = (b - 1) * 10 + 1
        end   = b * 10
    ...
    # Summary row Catch% — overall pooled rate as plain text
    if total_runs_sum > 0:
        overall_pct = round(total_caught_sum * 100 / total_runs_sum)
        summary_pct_val = f"{overall_pct}% (pooled)"
    else:
        summary_pct_val = "-"
    cell = ws.cell(row=summary_row, column=col_pct, value=summary_pct_val)
    style_cell(cell, fill=SUMMARY_FILL, font=SUMMARY_FONT, align=CENTER)
