...
# ============================================================
# Robust llvm-symbolizer setup
# ============================================================

CLANGPP_REAL=$(realpath "$CLANGPP" 2>/dev/null)

if [[ -n "$CLANGPP_REAL" ]]; then
    CLANG_BIN_DIR="$(dirname "$CLANGPP_REAL")"
else
    CLANG_BIN_DIR="$(dirname "$(which clang++)")"
fi

SYMBOLIZER="$CLANG_BIN_DIR/llvm-symbolizer"

if [[ -x "$SYMBOLIZER" ]]; then
    export ASAN_SYMBOLIZER_PATH="$SYMBOLIZER"
    export PATH="$CLANG_BIN_DIR:$PATH"
else
    echo "WARNING: llvm-symbolizer not found at:"
    echo "  $SYMBOLIZER"
fi
...
extract_fault_thread() {
    ...
    if [[ -z "$dyn_tid" ]]; then
        dyn_tid=$(echo "$asan_err" \
            | grep -oE 'ERROR: AddressSanitizer: SEGV.*T[0-9]+\)' \
            | head -1 \
            | grep -oE 'T[0-9]+\)' \
            | tr -d ')')
    fi
    ...
    # ============================================================
    # Extract first user source location
    # Example:
    # foo.cpp:29:5
    # ============================================================
    local loc

    loc=$(echo "$creation_block" \
        | grep -oE '[^ ]+\.cpp:[0-9]+:[0-9]+' \
        | head -1)

    # ============================================================
    # Convert to STATIC thread identifier
    # ============================================================
    if [[ -n "$loc" ]]; then
        local line
...
analyse_benchmark() {
    ...
    # dynamic tid -> static site
    declare -A dyn_to_static_map

    # unresolved dynamic tids waiting for repair
    declare -A unresolved_tid_count
    ...
    while (( valid_runs < RUNS && total_attempts < max_attempts )); do
        ...
        if [[ "$asan_class" == "ASAN_CAUGHT" || "$asan_class" == "ASAN_WARNING" ]]; then
            ...
            # ---- Extract faulting thread and tally ----
            # ============================================================
# Extract thread info
# ============================================================
thread_info=$(extract_fault_thread "$stderr_content")

dyn_tid="${thread_info%%|*}"
static_site="${thread_info##*|}"

# ============================================================
# COMPLETE FAILURE
# ============================================================
if [[ "$dyn_tid" == "UNKNOWN" ]]; then
    ...
    thread_catch_map["UNKNOWN"]=$(
        ( ${thread_catch_map["UNKNOWN"]:-0} + 1 )
    )

# ============================================================
# STATIC SITE RESOLVED
# ============================================================
elif [[ -n "$static_site" ]]; then

    # Learn mapping
    dyn_to_static_map["$dyn_tid"]="$static_site"

    # Count immediately
    thread_catch_map["$static_site"]=$(
        ( ${thread_catch_map["$static_site"]:-0} + 1 )
    )

# ============================================================
# Dynamic tid known but symbols missing
# ============================================================
else

    # Previously learned mapping?
    if [[ -n "${dyn_to_static_map[$dyn_tid]}" ]]; then

        repaired_site="${dyn_to_static_map[$dyn_tid]}"

        thread_catch_map["$repaired_site"]=$(
            ( ${thread_catch_map["$repaired_site"]:-0} + 1 )
        )

    else

        # Delay until future runs resolve symbols
        unresolved_tid_count["$dyn_tid"]=$(
            ( ${unresolved_tid_count[$dyn_tid]:-0} + 1 )
        )

        mkdir -p "$OUTDIR/unresolved_dynamic_tid"

        unresolved_file="$OUTDIR/unresolved_dynamic_tid/${slug}_${dyn_tid}_run${valid_runs}.txt"

        {
            echo "======================================================"
            echo "FILE      : $filename"
            echo "RUN       : $valid_runs"
            echo "DYNAMIC TID : $dyn_tid"
            echo "TIMESTAMP : $(date)"
            echo "======================================================"
            echo ""
            echo "$stderr_content"
...
# ============================================================
# Resolve delayed unresolved dynamic thread ids
# ============================================================
for tid in "${!unresolved_tid_count[@]}"; do

    cnt="${unresolved_tid_count[$tid]}"

    if [[ -n "${dyn_to_static_map[$tid]}" ]]; then

        repaired_site="${dyn_to_static_map[$tid]}"

        thread_catch_map["$repaired_site"]=$(
            ( ${thread_catch_map["$repaired_site"]:-0} + cnt )
        )

    else

        thread_catch_map["UNKNOWN"]=$(
            ( ${thread_catch_map["UNKNOWN"]:-0} + cnt )
        )
    fi
done
    rm -f "$binary"
...
for cat_name, dir_fragments in PAPER_CATEGORIES:
    ...
    for row_idx, s in enumerate(sorted(rows, key=lambda x: x["fn"]), 2):
        ...
        if not is_skipped and s["valid_runs"] > 0:
            caught_cell = get_column_letter(col_total) + str(row_idx)
            runs_cell   = get_column_letter(col_runs)  + str(row_idx)
            cell = ws.cell(row=row_idx, column=col_pct,
                           value=f"=IF({runs_cell}>0,{caught_cell}/{runs_cell},0)")
            cell.number_format = "0.0%"
        else:
            cell = ws.cell(row=row_idx, column=col_pct, value="-")
        style_cell(cell, fill=TOTAL_FILL if not is_skipped else SKIP_FILL,
                   font=BOLD_FONT, align=CENTER)
